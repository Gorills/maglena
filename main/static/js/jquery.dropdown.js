...
(function ($, window, document, undefined) {
  ...
  $.extend(Dropdown.prototype, {
    ...
    select: function (item) {
      var self = this;
      var opt = self.opt,
          inst = self.inst; // Get item

      item = self.getItem(item);
      if (!item) return false; // Open menu

      if (opt.nested && item.children.menu) return self.openMenu(item.children.menu); // Deselect
    ...
    open: function (menu) {
      ...
      var opt = self.opt,
          inst = self.inst,
          elem = self.elems; // Already open/opening
    ...
    close: function (menu) {
      ...
      var opt = self.opt,
          inst = self.inst,
          elem = self.elems; // Already closed/closing
    ...
    openMenu: function (menu, noAnimation) {
      var self = this;
      var opt = self.opt,
          inst = self.inst; // Already opening
      ...
      if (current) current.elem.transition({
        x: '-100%'
      }, speed);
    ...
    closeMenu: function (menu, noAnimation) {
      var self = this;
      var opt = self.opt,
          inst = self.inst; // Already closing
    ...
    resize: function (menu, noAnimation) {
      var self = this,
          opt = self.opt,
          inst = self.inst,
          wrapper = self.elems.menuWrapper; // Already resizing
    ...
    focus: function (item) {
      var self = this,
          opt = self.opt,
          inst = self.inst,
          cls = self.cls; // Remove current focus
    ...
    getMenu: function (menu) {
      var self = this,
          inst = self.inst,
          elem = self.inst; // Check if this is an item
      ...
      if (typeof menu === 'string') {
        // Main menu
        if ('main' == menu) menu = inst.menus[inst.menuMain]; // Object
        else if (inst.menus[menu]) menu = inst.menus[menu]; // Element
          else if (elem.dropdown.find('#' + menu)) menu = elem.dropdown.find('#' + menu);
      }
    ...
    getItem: function (item) {
      var self = this,
          inst = self.inst,
          elem = self.elems; // Get by ID

      if (typeof item === 'string') {
        // Object
        if (inst.items[item]) item = inst.items[item]; // Element
        else if (elem.dropdown.find('#' + item)) item = elem.dropdown.find('#' + item);
      }
    ...
    toggleText: function (text) {
      var self = this,
          elem = self.elems; // Get toggle

      var $toggle = elem.toggleButton,
          $text = elem.toggleText; // Store original
    ...
    toggleTextMulti: function (text) {
      ...
      if (-1 !== index) vals.splice(index, 1); // Add text
      else vals.push(text); // Get new text
    ...
    _collisionValues: function (menu, resize) {
      var self = this,
          opt = self.opt,
          inst = self.inst,
          elem = self.elems; // Create collision object
    ...
    _build: function () {
      var self = this,
          opt = self.opt,
          elem = self.elems,
          cls = self.cls; // Loop through each template
    ...
    _buildItem: function (item) {
      ...
      var elems = {};
      var names = ['menuItem', 'menuLink', 'menuText', 'menuDivider', 'menuLabel'];
      $.each(names, function (i, name) {
        // Create element
        if (tpls[name]) {
          elems[name] = $(tpls[name]); // Add classes
    ...
    _populateList: function ($target) {
      ...
      $target.children().each(function () {
        ...
        if ($this.children('ul, ol').length) {
          ...
          if ($this.data('dropdown-selected') || $this.hasClass(self._cls.selected)) item.selected = true;
        } // Add to items
    ...
    _scrollSelected: function (menu, resize) {
      ...
      if (!inst.open) elem.menuWrapper.show().css({
        opacity: 0
      }); // Show the menu if needed

      if (!menu.open) menu.elem.show().css({
        opacity: 0
      }); // Get list
      ...
      if (!inst.open) elem.menuWrapper.css({
        display: '',
        opacity: ''
      });
      if (!menu.open) menu.elem.css({
        display: '',
        opacity: ''
      });
    },
